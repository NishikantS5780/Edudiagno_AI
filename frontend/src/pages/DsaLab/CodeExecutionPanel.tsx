  ...
  const handleCodeRun = () => {
    ...
    fetch(
      ...
      .then((body) => {
        ...
        if (!body || !Array.isArray(body) || !body[0] || !body[0].output || !body[0].output.data || !body[0].output.data.taskId) {
      ...
      fetch(
        ...
        .then((body) => {
          ...
          if (!firstResponse || !firstResponse.output) {
            console.error('First response element missing output:', firstResponse);
          ...
          if (data.sourceCodeAsBase64UrlEncoded) {
            const decodedCode = atob(data.sourceCodeAsBase64UrlEncoded.replace(/-/g, "+").replace(/_/g, "/"));
          ...
          if (data.codingTaskStatus === 'Pending') {
            console.log('Task is still pending, checking again in 2 seconds...');
          ...
          let compiler_syntax_error = runResult.compilerOutputAfterCompilationBase64UrlEncoded
            ?.replace(/-/g, "+")
            ?.replace(/_/g, "/");
          ...
          const compilationStatus = run_status === "Success" ? "Compiled Successfully" : 
                                 run_status === "Error" ? "Compilation Failed" : 
                                 "Compiling...";
              ...
              <TabsContent value="result" className="p-4 space-y-4">
                ...
                {codeError && (
                  <div className="text-red-500 bg-[#27272a] p-3 rounded-lg">
                    <h3 className="font-semibold mb-1 text-red-400">Runtime Error:</h3>
                    <pre className="whitespace-pre-wrap font-mono text-sm">{codeError}</pre>
                  </div>
                )}
                {syntaxError && (
                  <div className="text-red-500 bg-[#27272a] p-3 rounded-lg">
                    <h3 className="font-semibold mb-1 text-red-400">Compilation Error:</h3>
                    <pre className="whitespace-pre-wrap font-mono text-sm">{syntaxError}</pre>
                  </div>
                )}
                <div className="flex items-center gap-2 bg-[#27272a] p-3 rounded-lg">
                  <span className="font-semibold text-gray-400">Compilation Status:</span>
                  <span className={`${!syntaxError ? 'text-green-400' : 'text-red-500'} font-mono`}>
                    {!syntaxError ? 'Successfully Compiled' : 'Compilation Failed'}
                  </span>
                </div>
                <div className="flex items-center gap-2 bg-[#27272a] p-3 rounded-lg">
                  <span className="font-semibold text-gray-400">Execution Status:</span>
                  <span className={`${runStatus === 'successful' ? 'text-green-400' : 'text-red-500'} font-mono`}>
                    {runStatus === 'successful' ? 'Successfully Executed' : 
                     runStatus === 'wrong-answer' ? 'Wrong Answer' :
                     runStatus === 'compilation-error' ? 'Compilation Error' :
                     runStatus === 'error' ? 'Runtime Error' :
                     runStatus || 'Not Executed'}
                  </span>
                </div>
                <div className="flex items-center gap-2 bg-[#27272a] p-3 rounded-lg">
                  <span className="font-semibold text-gray-400">Expected Output:</span>
                  <span className="text-yellow-400 font-mono">{expectedOutput}</span>
                </div>
              </TabsContent>
